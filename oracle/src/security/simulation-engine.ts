...
export class SecurityEngine {
  ...
  async runSecurityAudit(protocolState: any): Promise<RiskScore> {
    const results = await Promise.all(this.scenarios.map((s) => s.run(protocolState)));

    const aggregateScore = results.reduce((acc, res) => acc + res.riskScore, 0) / results.length;
    ...
    return {
      ...
      breakdown: {
        oracleRisk:
          results.find((r) => r.scenarioName === 'Oracle Price Manipulation')?.riskScore || 0,
        liquidityRisk:
          results.find((r) => r.scenarioName === 'Liquidation Cascade')?.riskScore || 0,
