...
`public-functions.json` lists the benchmark operations that must be present in every complete report. The CI gate fails when:

1. A required operation is missing from `benchmark-results.json`
2. An operation exceeds its hard instruction budget
3. An operation regresses by more than 10% compared with `baseline.json`

The gate and dashboard are generated by:

```bash
python3 scripts/gas_benchmark_report.py \
  --results stellar-lend/benchmark-results.json \
  --baseline stellar-lend/benchmarks/baseline.json \
  --coverage stellar-lend/benchmarks/public-functions.json \
  --dashboard stellar-lend/benchmark-dashboard.md \
  --history stellar-lend/benchmarks/history.jsonl \
  --history-out stellar-lend/benchmark-history.jsonl \
...
- `benchmark-results.json` with raw per-operation measurements
- `benchmark-dashboard.md` with contract summaries, missing coverage, regressions, storage usage, and cross-contract calls
- `benchmark-history.jsonl` with the historical trend plus the latest run

Commit `benchmarks/baseline.json` after intentional gas changes. Commit `benchmarks/history.jsonl` only when maintainers want a new long-lived trend point in the repository.
...
- Fails the PR if a required operation is missing, any operation exceeds its gas budget, or any operation regresses by more than 10%
- Uploads `benchmark-results.json`, `benchmark-dashboard.md`, `benchmark-history.jsonl`, `benchmark-results.md`, and `benchmarks/history.json` as CI artifacts (retained 90 days)
- Posts the Markdown dashboard and a summary table to the GitHub Actions step summary
