...
fn test_migration_stellar_other() {
    ...
    assert_eq!(token.balance(&contract_id), 0); // No stranded funds in Hub!
    assert_eq!(token.balance(&lending), 500); // Funds delivered to destination lending pool
    assert_eq!(client.verify_migration(&migration_id), Ok(true));
