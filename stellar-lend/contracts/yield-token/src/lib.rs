...
impl YieldToken {
    pub fn initialize(
        ...
        if env.storage().instance().has(&DataKey::Admin) {
            return Err(YieldTokenError::AlreadyInitialized);
        }
        admin.require_auth();
    ...
    pub fn transfer_from(
        ...
        if current_allowance < amount {
            return Err(YieldTokenError::InsufficientAllowance);
        }
    ...
    pub fn mint(
        ...
        let splitter: Address = env
            ...
            .ok_or(YieldTokenError::Unauthorized)?;
        splitter.require_auth();

        if amount <= 0 {
            return Err(YieldTokenError::InvalidAmount);
        }
        let active = env
            .storage()
            .instance()
            ...
            .unwrap_or(false);
        if !active {
            return Err(YieldTokenError::TokenNotActive);
        }
    ...
    pub fn burn(
        ...
        let mut balance: i128 = env
            ...
            .unwrap_or(0);
        if balance < amount {
            return Err(YieldTokenError::InsufficientBalance);
        }
        balance = balance
            .checked_sub(amount)
            .ok_or(YieldTokenError::Overflow)?;
    ...
    pub fn set_active(
        ...
        if admin != stored_admin {
            return Err(YieldTokenError::Unauthorized);
        }
        admin.require_auth();
